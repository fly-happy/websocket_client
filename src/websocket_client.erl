...
-export([start_link/3, start_link/4, start_link/5, cast/2, send/2]).
-export([ws_client_init/8]).

-type opt() ::
    {async_start, boolean()} | {extra_headers, [{string() | binary(), string() | binary()}]}.
-type opts() :: [opt()].

%% @doc Start the websocket client
-spec start_link(URL :: string() | binary(),
                 Handler :: module(),
                 HandlerArgs :: list()) ->
                    {ok, pid()} | {error, term()}.
...
start_link(URL, Handler, HandlerArgs, Opts) ->
    start_link(URL, Handler, HandlerArgs, Opts, undefined).

start_link(URL, Handler, HandlerArgs, Opts, PreconnectedSock) when is_binary(URL) ->
    start_link(erlang:binary_to_list(URL), Handler, HandlerArgs, Opts, PreconnectedSock);
start_link(URL, Handler, HandlerArgs, Opts, PreconnectedSock) when is_list(Opts) ->
...
-spec ws_client_init(Handler :: module(),
                     Protocol :: websocket_req:protocol(),
                     Host :: string(),
                     ...
                     Opts :: opts(),
                     PreconnectedSock ::
                         undefined | {ssl, ssl:ssl_socket()} | {tcp, gen_tcp:socket()}) ->
                        no_return().
ws_client_init(Handler, Protocol, Host, Port, Path, Args, Opts, PreconnectedSock) ->
    Transport =
        case Protocol of
            wss ->
                ssl;
    ...
    SockReply =
        case {Transport, PreconnectedSock} of
            {ssl, {ssl, SslSocket}} ->
                {ok, SslSocket};
            {ssl, {tcp, TcpSocket}} ->
                ssl:connect(TcpSocket,
                            [{verify, verify_none},
                             {log_level, none},
                             {mode, binary},
                             {active, false},
    ...
    case websocket_handshake(WSReq, ExtraHeaders) of
        ...
        {ok, Buffer} ->
            ...
            {ok, HandlerState, KeepAlive} =
                case Handler:init(Args, WSReq) of
                    {ok, HS} ->
                        {ok, HS, infinity};
                    {ok, HS, KA} ->
                        {ok, HS, KA}
            ...
            websocket_loop(websocket_req:set([{keepalive, KeepAlive}, {keepalive_timer, KATimer}],
                                             WSReq),
                           HandlerState,
                           <<>>)
    end.
...
-spec websocket_handshake(WSReq :: websocket_req:req(), [{string(), string()}]) ->
...
-spec receive_handshake(Buffer :: binary(), Transport :: module(), Socket :: term()) ->
                           {ok, binary()}.
...
%% @doc Main loop
-spec websocket_loop(WSReq :: websocket_req:req(),
                     HandlerState :: any(),
                     Buffer :: binary()) ->
                        ok.
...
handle_websocket_message(WSReq, HandlerState, Buffer, Message) ->
    [Handler, Remaining, Socket] = websocket_req:get([handler, remaining, socket], WSReq),
    case Message of
        keepalive ->
            ...
            KATimer =
                erlang:send_after(
                    websocket_req:keepalive(WSReq), self(), keepalive),
        ...
        {_TransportType, Socket, Data} ->
            case Remaining of
                undefined ->
                    retrieve_frame(WSReq, HandlerState, <<Buffer/binary, Data/binary>>);
                _ ->
                    retrieve_frame(WSReq,
                                   HandlerState,
                                   websocket_req:opcode(WSReq),
                                   Remaining,
                                   Data,
                                   Buffer)
            end;
        ...
        _ ->
            case Reason of
                normal ->
                    ok;
                _ ->
                    error_info(Handler, {exit, Reason, []}, HandlerState)
            end,
            exit(Reason)
    catch
...
error_info(Handler, {Class, Reason, Stacktrace}, State) ->
    error_logger:error_msg("** Websocket handler ~p terminating~n"
                           "** Class: ~p~n"
                           "** Reason: ~p~n"
...
retrieve_frame(WSReq, HandlerWSReq, <<0:4, Opcode:4, 0:1, Len:7, Rest/bits>>)
    when Len < 126 ->
...
retrieve_frame(WSReq, HandlerWSReq, <<0:4, Opcode:4, 0:1, 126:7, Len:16, Rest/bits>>)
    when Len > 125, Opcode < 8 ->
    WSReq1 = set_continuation_if_empty(WSReq, Opcode),
    WSReq2 = websocket_req:fin(0, WSReq1),
    retrieve_frame(WSReq2, HandlerWSReq, Opcode, Len, Rest, <<>>);
%% @doc Start or continue continuation payload with length a 64 bit int
retrieve_frame(WSReq, HandlerWSReq, <<0:4, Opcode:4, 0:1, 127:7, 0:1, Len:63, Rest/bits>>)
    when Len > 16#ffff, Opcode < 8 ->
    WSReq1 = set_continuation_if_empty(WSReq, Opcode),
    WSReq2 = websocket_req:fin(0, WSReq1),
    retrieve_frame(WSReq2, HandlerWSReq, Opcode, Len, Rest, <<>>);
%% @doc Length is less 126 bytes
retrieve_frame(WSReq, HandlerWSReq, <<1:1, 0:3, Opcode:4, 0:1, Len:7, Rest/bits>>)
    when Len < 126 ->
    WSReq1 = websocket_req:fin(1, WSReq),
    retrieve_frame(WSReq1, HandlerWSReq, Opcode, Len, Rest, <<>>);
%% @doc Length is a 2 byte integer
retrieve_frame(WSReq, HandlerWSReq, <<1:1, 0:3, Opcode:4, 0:1, 126:7, Len:16, Rest/bits>>)
    when Len > 125, Opcode < 8 ->
    WSReq1 = websocket_req:fin(1, WSReq),
    retrieve_frame(WSReq1, HandlerWSReq, Opcode, Len, Rest, <<>>);
...
retrieve_frame(WSReq, HandlerState, Opcode, Len, Data, Buffer) ->
    ...
    case OpcodeName of
        ...
        _ when Opcode < 8, Continuation =/= undefined, Fin == 1 ->
            ...
            try Handler:websocket_handle({ContinuationOpcodeName, DefragPayload},
                                         WSReq2,
                                         HandlerState)
            of
                HandlerResponse ->
        ...
        _ ->
            try Handler:websocket_handle({OpcodeName, FullPayload}, WSReq, HandlerState) of
                HandlerResponse ->
